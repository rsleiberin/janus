...
def test_logger_initialization(logger):
    ...
    assert isinstance(logger.logger, logging.Logger), "CentralizedLogger should initialize a Logger instance."
    assert len(logger.logger.handlers) > 0, "Logger should have at least one handler."
    handler = logger.logger.handlers[0]
    assert isinstance(handler, logging.StreamHandler), "The handler should be a StreamHandler."
...
def test_log_to_console(logger):
    ...
    with patch.object(logger.logger, 'info') as mock_info:
        logger.log_to_console("INFO", "Test console log message.", context="TestContext")
        mock_info.assert_called_once_with("Test console log message. | Context: {\"context\": \"TestContext\"}")
...
def test_invalid_log_level(logger):
    ...
    with patch.object(logger.logger, 'warning') as mock_warning:
        ...
        mock_warning.assert_called_once_with("Invalid log level 'INVALID': Test invalid log level.")
...
def test_log_to_db_handles_exceptions(logger):
    ...
    with patch('backend.db.session.add', side_effect=Exception("DB error")) as mock_add, \
         patch.object(logger.logger, 'error') as mock_error, \
         patch('backend.db.session.rollback') as mock_rollback:
        logger.log_to_db(
            level="ERROR",
            message="Failing DB log message",
            module="TestModule"
        )
        assert mock_add.called, "The log entry should attempt to be added to the database session."
        assert mock_rollback.called, "The database session should be rolled back on error."
...
def test_format_message(logger):
    ...
    formatted_message = logger.format_message("Base message", key1="value1", key2="value2")
    ...
    assert plain_message == "Plain message", "Message formatting should handle plain messages without context."
