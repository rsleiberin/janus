...
def test_create_image():
    ...
    with pytest.raises(ImageCreationError, match="Failed to create image: .*"):
...
def test_get_by_id():
    ...
    with patch(
        ...
        mock_console_log.assert_called_with(
            ...
            found=True,  # Match actual argument used in the implementation.
        )
...
def test_update_image():
    ...
    with patch(
        ...
        mock_console_log.assert_called_with(
            ...
            updates=updated_data,  # Match actual argument used in the implementation.
        )
...
def test_delete_image():
    ...
    with patch(
        ...
        mock_console_log.assert_called_with(
            "INFO",
            f"Deleted image with ID: {image.id}",  # Match actual message
        )
...
def test_get_all_images():
    """
    Test retrieving all image records.
    """
    ImageHelpers.create(
        {
            "filename": "img1.png",
    ...
    with patch(
        ...
        assert len(all_images) == 2, f"Expected 2 images, but got {len(all_images)}."


@pytest.mark.usefixtures("function_db_setup")
def test_filter_by():
    """
    Test filtering images by a specific field.
    """
    ImageHelpers.create(
        {
    ...
    with patch(
        ...
        mock_console_log.assert_called_with(
            "INFO",
            "Filtered images by color_type",
            value="RGB",
            count=1,
        )

        assert len(rgb_images) == 1, f"Expect 1 RGB img, found {len(rgb_images)}."


@pytest.mark.usefixtures("function_db_setup")
def test_count_images():
    """
    Test counting the total number of image records.
    """
    ImageHelpers.create(
        {"filename": "count1.png", "width": 100, "height": 100, "user_id": 1}
    )
    ImageHelpers.create(
        {"filename": "count2.png", "width": 200, "height": 200, "user_id": 1}
    )
...
def test_get_images_by_metadata():
    """
    Test retrieving images by specific metadata.
    """
...
def test_exists_image():
    """
    Test checking if an image with a specific ID exists.
    """
...
def test_get_by_filename():
    """
    Test retrieving an image by filename.
    """
    filename = "search_by_filename.png"
    ImageHelpers.create(
        {"filename": filename, "width": 640, "height": 480, "user_id": 1}
    )
...
def test_get_images_by_date_range():
    """
    Test retrieving images created within a specific date range.
    """
    now = datetime.utcnow()
    ...
    with patch(
        ...
        results = ImageHelpers.get_images_by_date_range(
...
def test_get_images_by_size():
    """
    Test retrieving images larger than a specified size.
    """
    ImageHelpers.create(
    ...
    with patch(
        ...
        assert len(large_images) == 1, f"Expect 1 img >=1280x720, {len(large_images)}."
        assert large_images[0].filename == "big.png", "Wrong img return for size filter"


@pytest.mark.usefixtures("function_db_setup")
def test_update_image_error():
    """
    Test that ImageUpdateError is raised if an error occurs during the update process.
    """
    initial_data = {
        "filename": "update_error_test.png",
        "width": 300,
        "height": 300,
        "user_id": 1,
    }
