...
class AnalyticsHelpers:
    ...
    def create_analytics(data, session: Session = None):
        ...
        try:
            ...
            logger.log_to_console("ERROR", "Failed to create analytics entry", exception=e)
            logger.log_to_db(
                "ERROR",
            ...
            raise AnalyticsCreationError(f"Failed to create analytics entry: {str(e)}") from e
    ...
    def get_by_id(session: Session, analytics_id: int):
        ...
        try:
            ...
            if not analytics:
                raise AnalyticsNotFoundError(f"Analytics entry with ID {analytics_id} not found.")
            ...
            logger.log_to_console("ERROR", f"Failed to retrieve analytics entry with ID: {analytics_id}", exception=e)
            handle_database_error(e, module="analytics_helpers", meta_data={"analytics_id": analytics_id})
    ...
    def get_all_analytics(session: Session = None):
        ...
        try:
            ...
            logger.log_to_console("ERROR", "Failed to retrieve all analytics entries", exception=e)
            raise AnalyticsQueryError("Failed to retrieve all analytics entries.") from e
    ...
    def get_recent_analytics(session: Session = None, limit=10):
        ...
        try:
            ...
            logger.log_to_console("ERROR", f"Failed to retrieve recent analytics entries (limit {limit})", exception=e)
            raise AnalyticsQueryError(f"Failed to retrieve recent analytics entries (limit {limit}).") from e
    ...
    def delete_analytics(session: Session, analytics_id: int):
        ...
        try:
            ...
            logger.log_to_console("ERROR", f"Failed to delete analytics entry with ID: {analytics_id}", exception=e)
            raise AnalyticsDeletionError(f"Failed to delete analytics entry with ID {analytics_id}.") from e
    ...
    def count(session: Session = None):
        ...
        try:
            ...
            logger.log_to_console("ERROR", "Failed to count analytics entries", exception=e)
    ...
    def exists(session: Session, analytics_id: int):
        ...
        try:
            ...
            logger.log_to_console("ERROR", f"Failed to check existence of analytics entry with ID: {analytics_id}", exception=e)
            raise AnalyticsQueryError(f"Failed to check existence of analytics entry with ID {analytics_id}.") from e
