...
class ImageHelpers:
    ...
    def create(image_data):
        ...
        try:
            ...
            logger.log_to_console(
                "ERROR", f"Failed to create image: {str(e)}", image_data=image_data
            )
            logger.log_to_db(
                level="ERROR",
    ...
    def get_by_id(image_id):
        ...
        try:
            ...
            logger.log_to_console(
                "INFO", f"Retrieved image by ID: {image_id}", found=True
            )
            logger.log_to_db(
                level="INFO",
            ...
            raise handle_database_error(
                e, module="image_helpers", meta_data={"image_id": image_id}
            )
    ...
    def update(image_id, updated_data):
        ...
        try:
            ...
            db.session.commit()
            logger.log_to_console(
                "INFO", f"Updated image with ID: {image_id}", updates=updated_data
            )
            logger.log_to_db(
                level="INFO",
            ...
            raise ImageUpdateError(
                f"Failed to update image with ID {image_id}: {str(e)}"
            ) from e
    ...
    def delete(image_id):
        ...
        try:
            ...
            raise ImageDeletionError(
                f"Failed to delete image with ID {image_id}: {str(e)}"
            ) from e
    ...
    def filter_by(field, value):
        ...
        try:
            images = (
                db.session.query(Image).filter(getattr(Image, field) == value).all()
            )
            logger.log_to_console(
                "INFO", f"Filtered images by {field}", value=value, count=len(images)
            )
            logger.log_to_db(
                level="INFO",
            ...
            raise handle_database_error(
                e, module="image_helpers", meta_data={"field": field, "value": value}
            )
    ...
    def get_by_filename(filename):
        ...
        try:
            ...
            logger.log_to_console(
                "INFO", f"Retrieved image by filename: {filename}", found=True
            )
            logger.log_to_db(
                level="INFO",
            ...
            raise handle_database_error(
                e, module="image_helpers", meta_data={"filename": filename}
            )
    ...
    def get_images_by_date_range(start_date, end_date):
        ...
        try:
            images = (
                ...
                .filter(Image.created_at >= start_date, Image.created_at <= end_date)
                .all()
            )
            logger.log_to_console(
                "INFO",
    ...
    def get_images_by_size(min_width, min_height):
        ...
        try:
            images = (
                db.session.query(Image)
                .filter(Image.width >= min_width, Image.height >= min_height)
                .all()
            )
            logger.log_to_console(
                "INFO",
    ...
    def get_by_metadata(metadata):
        ...
        try:
            ...
            raise handle_database_error(
                e, module="image_helpers", meta_data={"metadata": metadata}
            )
