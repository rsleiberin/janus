...
class UserHelpers:
    ...
    def create(user_data):
        ...
        try:
            ...
            logger.log_to_console(
                "ERROR", "Failed to create user", error=str(e), user_data=user_data
            )
            db.session.rollback()
    ...
    def get_by_id(user_id):
        ...
        try:
            ...
            logger.log_to_console(
                "INFO", "Fetched user by ID", user_id=user_id, found=True
            )
            return user
        except InvalidRequestError as e:
            logger.log_to_console(
                "ERROR", "Invalid query for get_by_id", error=str(e), user_id=user_id
            )
            raise UserQueryError(f"Invalid query in get_by_id: {str(e)}") from e
        except SQLAlchemyError as e:
            logger.log_to_console(
                "ERROR", "Failed to fetch user by ID", error=str(e), user_id=user_id
            )
            raise UserQueryError(
                f"Query error while fetching user by ID: {user_id}"
            ) from e
    ...
    def get_by_email(email):
        ...
        try:
            ...
            logger.log_to_console(
                "INFO", "Fetched user by email", email=email, found=True
            )
            return user
        except InvalidRequestError as e:
            logger.log_to_console(
                "ERROR", "Invalid query for get_by_email", error=str(e), email=email
            )
            raise UserQueryError(f"Invalid query in get_by_email: {str(e)}") from e
        except SQLAlchemyError as e:
            logger.log_to_console(
                "ERROR", "Failed to query user by email", error=str(e), email=email
            )
            raise UserQueryError(
                f"Query error while fetching user by email: {email}"
            ) from e
    ...
    def delete(user_id):
        ...
        try:
            ...
            if user:
                ...
                db.session.commit()
                logger.log_to_console(
                    "INFO", "User deleted successfully", user_id=user_id
                )
            else:
            ...
            logger.log_to_console(
                "ERROR", "Failed to delete user", error=str(e), user_id=user_id
            )
            db.session.rollback()
    ...
    def exists(user_id):
        ...
        try:
            ...
            logger.log_to_console(
                "ERROR", "Failed to check if user exists", error=str(e), user_id=user_id
            )
            raise UserQueryError(
                f"Error checking existence of user ID: {user_id}"
            ) from e
