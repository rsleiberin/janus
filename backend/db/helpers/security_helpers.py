...
class SecurityHelpers:
    ...
    def create_security_entry(user_id, action):
        ...
        try:
            ...
            logger.log_to_console(
                "INFO", "Created security entry", user_id=user_id, action=action
            )
            ...
            logger.log_to_console(
                "ERROR",
                "Failed to create security entry",
                user_id=user_id,
                action=action,
                error=str(e),
            )
            ...
            raise handle_database_error(
                ...
                meta_data={"user_id": user_id, "action": action},
            )
    ...
    def get_security_by_id(security_id):
        ...
        try:
            ...
            if not security_entry:
                raise SecurityEntryNotFoundError(
                    f"Security entry with ID {security_id} not found."
                )
            logger.log_to_console(
                "DEBUG",
                "Fetched security entry by ID",
                security_id=security_id,
                found=bool(security_entry),
            )
            return security_entry
            ...
            logger.log_to_console(
                "ERROR",
                "Failed to fetch security entry by ID",
                security_id=security_id,
                error=str(e),
            )
            raise handle_database_error(
    ...
    def get_security_by_user(user_id):
        ...
        try:
            security_entries = (
                db.session.query(Security).filter_by(user_id=user_id).all()
            )
            logger.log_to_console(
                "DEBUG",
                "Fetched security entries by user",
                user_id=user_id,
            ...
            logger.log_to_console(
                "ERROR",
                "Failed to fetch security entries by user",
                user_id=user_id,
                error=str(e),
            )
            raise SecurityQueryError(
                f"Failed to fetch security entries for user_id={user_id}"
            ) from e
    ...
    def get_recent_security_entries(limit=10):
        ...
        try:
            ...
            logger.log_to_console(
                "DEBUG",
                "Fetched recent security entries",
                limit=limit,
                count=len(security_entries),
            )
            return security_entries
            ...
            logger.log_to_console(
                "ERROR",
                "Failed to fetch recent security entries",
                limit=limit,
                error=str(e),
            )
            raise handle_database_error(
    ...
    def delete_security_entry(security_id):
        ...
        try:
            ...
            if not security_entry:
                logger.log_to_console(
                    "WARNING",
                    "Security entry not found for deletion",
                    security_id=security_id,
                )
                raise SecurityEntryNotFoundError(
                    f"Security entry with ID {security_id} not found for deletion."
                )
            db.session.delete(security_entry)
            db.session.commit()
            logger.log_to_console(
                "INFO", "Deleted security entry", security_id=security_id
            )
        except Exception as e:
            db.session.rollback()
            logger.log_to_console(
                "ERROR",
                "Failed to delete security entry",
                security_id=security_id,
                error=str(e),
            )
            raise SecurityEntryDeletionError(
                f"Failed to delete security entry with ID {security_id}"
            ) from e
    ...
    def count():
        ...
        try:
            ...
            logger.log_to_console(
                "ERROR", "Failed to count security entries", error=str(e)
            )
    ...
    def exists(security_id):
        ...
        try:
            exists = (
                db.session.query(Security).filter_by(id=security_id).first() is not None
            )
            logger.log_to_console(
                "DEBUG",
                "Checked if security entry exists",
                security_id=security_id,
                exists=exists,
            )
            return exists
            ...
            logger.log_to_console(
                "ERROR",
                "Failed to check if security entry exists",
                security_id=security_id,
                error=str(e),
            )
            raise SecurityQueryError(
                f"Failed to check existence of security entry with ID {security_id}."
            ) from e
